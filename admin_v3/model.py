...
class CtaUnifiedMarginRebalance(db.Model):
    ...
    hedge_market = Column(VARCHAR(32),
                          ...
                          comment='半套市场类型')
    buy_mode = Column(VARCHAR(32),
                      nullable=False,
                      server_default=text("'cash'"),
                      comment='底仓买入模式')
    margin_side_effect_type = Column(VARCHAR(64),
                                     nullable=False,
                                     server_default=text("''"),
                                     comment='统一账户杠杆买入方式')
    target_quote_usd = Column(DECIMAL(20, 4),
                              nullable=False,
    ...
    last_buy_order_id = Column(VARCHAR(128),
                               ...
                               server_default=text("''"),
                               comment='最近买币订单ID')
    last_borrow_asset = Column(VARCHAR(64),
                               nullable=False,
                               server_default=text("''"),
                               comment='最近借款资产')
    last_borrow_amount = Column(DECIMAL(20, 8),
                                nullable=False,
                                server_default=text("'0.00000000'"),
                                comment='最近借款数量')
    last_executed_base_qty = Column(DECIMAL(20, 8),
                                    nullable=False,
                                    server_default=text("'0.00000000'"),
                                    comment='最近成交基础币数量')
    base_wallet_source = Column(VARCHAR(64),
                                nullable=False,
                                server_default=text("'MARGIN_OR_SPOT'"),
                                comment='底仓钱包来源')
